...
public class WallSmashTrigger : MonoBehaviour
{
    ...
    [SerializeField] private GameObject camToShutdown;    // Le monstre
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player") && FinalKey.hasFinalKey/* && !isChasing*/)
        {
            ...
            finalChase = enemy.GetComponent<FinalChase>();

            if (booCam.booVisionActive == true)
            {
                booCam.DeactivateBooVision();
                Debug.Log("BooVision est desactivé !");
            }
            ...
            camToShutdown.SetActive(false);

            finalChase.StartChase();
